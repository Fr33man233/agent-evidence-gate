...
interface Arguments { manifest?: string; trace?: string; receipts?: string; repo?: string; json?: string; markdown?: string; }
function parseArguments(values: string[]): Arguments {
  ...
  for (let index = 1; index < values.length; index += 2) {
    ...
    if (key === "--manifest") output.manifest = value; else if (key === "--trace") output.trace = value; else if (key === "--receipts") output.receipts = value; else if (key === "--repo") output.repo = value; else if (key === "--json") output.json = value; else if (key === "--markdown") output.markdown = value; else throw new Error("usage");
  }
  return output;
}
...
function main(): number {
  ...
  let profile: ProfileName = "local";
  try {
    if (!args.manifest || !args.trace || !args.receipts) throw new Error("missing input");
    const report = verify({ manifestPath: args.manifest, tracePath: args.trace, receiptsPath: args.receipts, ...(args.repo ? { repoPath: args.repo } : {}) }); profile = report.profile; const json = renderJson(report); write(args.json, json); write(args.markdown, renderMarkdown(report)); process.stdout.write(json); return exitCode(report);
  } catch (error) {
