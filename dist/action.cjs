...
function assertJsonDepth(value, maximumDepth = LIMITS.jsonDepth) {
...
function assertSafeRepoPath(value, code2 = "AEG010") {
  if (typeof value !== "string" || value.length === 0 || value.length > LIMITS.pathLength) {
    throw new AegInputError(code2, "repository path is missing or outside the configured limit");
  }
  const normalized = value.replaceAll("\\", "/");
  if (normalized.startsWith("/") || /^[A-Za-z]:\//.test(normalized) || normalized.split("/").includes("..")) {
    throw new AegInputError(code2, "repository path is not relative and normalized");
  }
}
  ...
  const visit = (item) => {
    ...
    for (const [key, child] of Object.entries(item)) {
      ...
      if (lowerKey === "stdout" || lowerKey === "stderr") {
        ...
        if (keys.some((name) => !["summary", "bytes", "truncated"].includes(name)) || summary.summary !== void 0 && typeof summary.summary !== "string" || summary.bytes !== void 0 && typeof summary.bytes !== "number" || summary.truncated !== void 0 && typeof summary.truncated !== "boolean") throw new AegInputError("AEG050", "command output must be a bounded summary object");
...
// src/adapters.ts
function record(value, code2 = "AEG001") {
  if (value === null || typeof value !== "object" || Array.isArray(value)) throw new AegInputError(code2, "structured evidence must be an object");
  return value;
}
function string(value, code2 = "AEG001") {
  if (typeof value !== "string" || value.length === 0) throw new AegInputError(code2, "required evidence string is missing");
  return value;
}
function adaptEvidence(value) {
  ...
  const subject = record(input2.subject);
  const check = record(input2.check);
  const producer = record(input2.producer);
  const trust = input2.trust_context === void 0 ? {} : record(input2.trust_context);
  ...
  return {
    ...
    ...typeof input2.claimed_assurance === "string" ? { claimed_assurance: input2.claimed_assurance } : {}
  };
}
...
var profiles = /* @__PURE__ */ new Set(["local", "pr", "protected"]);
...
function parseBudget(value) {
  ...
  const number = (key) => {
    ...
    if (typeof item !== "number" || !Number.isFinite(item) || item < 0) throw new AegInputError("AEG001", "budget values must be non-negative numbers");
    return item;
  };
  if (budget.enforce !== void 0 && typeof budget.enforce !== "boolean") throw new AegInputError("AEG001", "budget enforce must be boolean");
  const output = {};
  const tokens = number("tokens");
  if (tokens !== void 0) output.tokens = tokens;
  const duration = number("duration_ms");
  if (duration !== void 0) output.duration_ms = duration;
  const toolCalls = number("tool_calls");
  if (toolCalls !== void 0) output.tool_calls = toolCalls;
  const retries = number("retries");
  if (retries !== void 0) output.retries = retries;
  if (typeof budget.enforce === "boolean") output.enforce = budget.enforce;
  return output;
}
function parseDependencyPolicy(value) {
  if (value === void 0) return void 0;
  if (value === null || typeof value !== "object" || Array.isArray(value)) throw new AegInputError("AEG001", "dependency_policy must be an object");
  const policy = value;
  if (policy.lockfile_required !== void 0 && typeof policy.lockfile_required !== "boolean") throw new AegInputError("AEG001", "dependency lockfile_required must be boolean");
  const names = (field) => policy[field] === void 0 ? void 0 : asStringArray(policy[field], `dependency_policy.${field}`);
  const output = {};
  if (typeof policy.lockfile_required === "boolean") output.lockfile_required = policy.lockfile_required;
  const allowed = names("allowed");
  if (allowed !== void 0) output.allowed = allowed;
  const denied = names("denied");
  if (denied !== void 0) output.denied = denied;
  return output;
}
function parseClaims(value) {
  ...
  return value.map((claim) => {
    if (claim === null || typeof claim !== "object" || Array.isArray(claim) || typeof claim.id !== "string") throw new AegInputError("AEG001", "claims require stable ids");
    const item = claim;
    if (ids.has(item.id)) throw new AegInputError("AEG001", "claim ids must be unique");
    ids.add(item.id);
    return { id: item.id, ...typeof item.status === "string" ? { status: item.status } : {} };
  });
}
function parseManifestText(text) {
  const document = (0, import_yaml.parseDocument)(text, { uniqueKeys: true, prettyErrors: false });
  if (document.errors.length > 0) throw new AegInputError("AEG001", "manifest is malformed or contains duplicate keys");
  const input2 = document.toJS();
  assertJsonDepth(input2);
  if (input2 === null || typeof input2 !== "object" || Array.isArray(input2)) throw new AegInputError("AEG001", "manifest must be an object");
  const value = input2;
  ...
  return {
    ...
    ...dependencyPolicy === void 0 ? {} : { dependency_policy: dependencyPolicy },
    ...budget === void 0 ? {} : { budget },
    ...value.test_surface !== void 0 ? { test_surface: asStringArray(value.test_surface, "test_surface") } : {},
    ...value.verifier_surface !== void 0 ? { verifier_surface: asStringArray(value.verifier_surface, "verifier_surface") } : {},
    ...claims === void 0 ? {} : { claims },
    ...value.exceptions !== void 0 ? { exceptions: asStringArray(value.exceptions, "exceptions") } : {}
  };
}
...
function matches(path, rules) {
  return rules.some((rule) => path === rule || path.startsWith(`${rule.replace(/\/$/, "")}/`));
}
function eventPaths(events, eventType) {
  return events.filter((event) => event.event_type === eventType).flatMap((event) => typeof event.data.path === "string" ? [event.data.path] : []);
}
function sumUsage(events, field) {
  const values = events.filter((event) => event.event_type === "model_usage").map((event) => event.data[field]).filter((value) => typeof value === "number" && Number.isFinite(value));
...
function evaluate(manifest, trace, evidence, gitFacts) {
  ...
  for (const path of eventPaths(trace, "file_written")) {
    try {
      assertSafeRepoPath(path);
    } catch {
      findings.push(finding("AEG010", "fail", "Trace contains an unsafe repository path."));
      continue;
    }
    if (matches(path, manifest.denied_paths) || manifest.allowed_paths.length > 0 && !matches(path, manifest.allowed_paths)) findings.push(finding("AEG010", "fail", "Trace writes outside the declared scope."));
    if (matches(path, manifest.sensitive_paths) && !manifest.exceptions?.includes(`sensitive-write:${path}`)) findings.push(finding("AEG030", "fail", "Trace writes a sensitive path without an exception."));
  }
  ...
  for (const path of reads) {
    if (matches(path, manifest.sensitive_paths) && !manifest.exceptions?.includes(`sensitive-read:${path}`)) findings.push(finding("AEG031", manifest.profile === "local" ? "warn" : "fail", "Trace reads a sensitive path without an exception."));
  }
  ...
  if (budget) {
    const metrics = [["tokens", "total_tokens"], ["duration_ms", "duration_ms"], ["tool_calls", "tool_calls"], ["retries", "attempt"]];
    for (const [limitKey, traceKey] of metrics) {
      ...
      if (used === void 0 || used === 0 && limitKey !== "retries" && !trace.some((event) => event.event_type === "model_usage")) {
        const enforced = manifest.profile === "protected" || budget.enforce === true;
        findings.push(finding("AEG050", enforced ? "fail" : "warn", "Declared resource budget has no observable usage."));
  ...
  if (budget && manifest.profile === "protected" && !["tokens", "duration_ms", "tool_calls", "retries"].some((key) => typeof budget[key] === "number")) findings.push(finding("AEG050", "fail", "Protected profile requires at least one declared resource budget dimension."));
...
function parseTraceText(text) {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    const value = item;
    if (value.schema_version !== "aeg-trace/v1" || typeof value.run_id !== "string" || typeof value.event_id !== "string" || typeof value.sequence !== "number" || typeof value.timestamp !== "string" || !eventTypes.has(value.event_type) || value.producer === null || typeof value.producer !== "object" || value.data === null || typeof value.data !== "object") throw new AegInputError("AEG002", "trace event is missing required structured fields");
    ...
    if (runId !== void 0 && runId !== value.run_id) throw new AegInputError("AEG002", "trace contains more than one run_id");
    runId = value.run_id;
...
function write(path, text) {
  ...
  (0, import_node_fs2.writeFileSync)(path, text, "utf8");
}
var code = 1;
try {
  ...
  write(input("json") || "gate-report.json", json);
  write(input("markdown") || "gate-report.md", renderMarkdown(report));
  process.stdout.write(json);
  code = exitCode(report);
} catch (error) {
  const reason = error instanceof Error && "code" in error && typeof error.code === "string" ? error.code : "AEG001";
  const report = preflightFailure("pr", reason);
  const json = renderJson(report);
  write(input("json") || "gate-report.json", json);
  write(input("markdown") || "gate-report.md", renderMarkdown(report));
  process.stdout.write(json);
