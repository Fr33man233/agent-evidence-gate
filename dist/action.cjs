...
function discoverReceiptPaths(inputPath) {
  ...
  let input2;
  try {
    input2 = (0, import_node_fs3.lstatSync)(root);
  } catch {
    throw new AegInputError("AEG003", "receipt input is unavailable");
  }
  if (input2.isSymbolicLink()) throw new AegInputError("AEG010", "receipt input must not be a link");
  if (input2.isFile()) {
    regularFile(root);
    return [root];
  }
  if (!input2.isDirectory()) throw new AegInputError("AEG003", "receipt input must be a file or store directory");
  let entries;
  try {
...
function write(path, text) {
  ...
  (0, import_node_fs5.writeFileSync)(path, text, "utf8");
}
var code = 1;
try {
